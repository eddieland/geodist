...
| -------- | ---- | ------------------ | ----- | ------ |
| P0 | Lock geometry surface for MVP | Decide on Multi* wrappers vs. endorsed `list[LineString|Polygon]`; add 3D LineString/Polygon (with validation/orientation checks) and update Shapely interop to preserve Z | Update `_geodist_rs.pyi`, docs, and tests; no silent Z drops | 📝 |
